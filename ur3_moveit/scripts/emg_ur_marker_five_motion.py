...
class STM_serial():
   def __init__(self):
      self.rx_data = 0
      self.tx_data = 0
      self.PORT_hand = '/dev/ttyUSB0'
      self.PORT_emg = '/dev/ttyACM0'
      self.BaudRate = 115200

      #print 'serial', serial.__version__
      self.ser_emg = serial.Serial(self.PORT_emg, self.BaudRate)
      self.ser_hand = serial.Serial(self.PORT_hand, self.BaudRate)
   ...
   def send_signal(self, motor_id, motor_val):
      ...
      self.ser_hand.write(bytes(bytearray([motor_id])))
      self.ser_hand.write(bytes(bytearray([motor_val])))        
...
class TestMove():
   ...
   def go_to_move(self, scale = 1.0):        # 로봇 팔: 마커 밑에 있는 물체 쪽으로 움직이기
      ...
      ###########
      self.serial_emg_enter(2)      
      ###########
      ##raw_input()     
      self.robot_arm.go(True)
      '''

   def serial_emg_enter(self, _num):
      """
      _num: serial emg mode. ex) seiral_emg_enter(0)
      """
      while True:
         self.read = stmser.ser_emg.readline().rstrip('\n')
         sp_str = self.read.split(',')

         if len(sp_str) == 11:
            self.data = sp_str[10]
            print sp_str, type(sp_str)
            print self.data, type(self.data)

            if self.data == str(_num):
               print ">>> serial_emg"
               break
...
if __name__=='__main__':
   ...
   #print "============ Press `Enter` to if plan is correct!! ..."
   print "============ first left motion!! ..."
   #raw_input()
   tm.go_to_move() 
   ##### left motion ######
   tm.serial_emg_enter(2)      
   ###########
   #raw_input()     
   ...
   print ">> Linear path planning 01 - pick the object, Please wait..."
   print "============ first down motion!! ..."
   ...
   ##### squeeze motion #####
   tm.serial_emg_enter(1)      
   ###########
   #raw_input()     
   ###### serial test ######
   #stm_ser.send_signal(1)
   stmser.hj_finger_control(stm_thumb_joint_id, full_grip)
   time.sleep(0.5)

   stmser.hj_finger_control(stm_index_id, full_grip)
   stmser.hj_finger_control(stm_middle_id, full_grip)
   stmser.hj_finger_control(stm_ring_id, full_grip)    
   stmser.hj_finger_control(stm_little_id, full_grip)
   stmser.hj_finger_control(stm_thumb_id, 0.003)
   time.sleep(3)
   #########################

   #x_offset = -0.1      y_offset = -0.0625      z_offset = -0.08
   linear_path = [+0.2, -0.2, +0.2]
   print ">> Linear path planning 02-z, Please wait..."
   print "============ first up motion!! ..."
   cartesian_plan, fraction = tm.plan_cartesian_z(linear_path[2]) # 모든 매개변수에 값을 줘야 함
   tm.display_trajectory(cartesian_plan)
   time.sleep(2)      
   #print "Press the Enter"
   ##### up motion ######
   tm.serial_emg_enter(4)      
   ###########
   #raw_input()      
   tm.execute_plan(cartesian_plan)
   ...
   print ">> Linear path planning 03-drop the object, Please wait..."
   cartesian_plan, fraction = tm.plan_cartesian_path(linear_path[0], linear_path[1], linear_path[2]) # 모든 매개변수에 값을 줘야 함
   tm.display_trajectory(cartesian_plan)
   time.sleep(0.5)      
   tm.execute_plan(cartesian_plan)
   ...
   #raw_input()     
   ###### serial test ######
   stmser.hj_finger_control(stm_index_id,  unGrip)
   stmser.hj_finger_control(stm_middle_id, unGrip)
   stmser.hj_finger_control(stm_ring_id,   unGrip)    
   stmser.hj_finger_control(stm_little_id, unGrip)
   stmser.hj_finger_control(stm_thumb_joint_id, unGrip)
   stmser.hj_finger_control(stm_thumb_id, unGrip)
   time.sleep(5)
   #########################
